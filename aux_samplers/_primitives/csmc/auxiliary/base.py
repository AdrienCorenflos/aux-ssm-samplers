"""
Implements the auxiliary particle Gibbs algorithm with guided proposals.
"""
...
from aux_samplers._primitives.csmc.standard import get_kernel as get_standard_kernel
...
def get_kernel(M0_factory: Callable[[Array, float], Distribution],
               ...
               Gt_factor: Callable[[Array, float], Potential],
               N: int,
               backward: bool = False,
    ...
    def kernel(key, state, delta):
        ...
        gt = Gt_factor(u[1:], sqrt_half_delta)
...
def get_coupled_kernel(M0_factory: Callable[[Array, float], Distribution],
                       G0_factory: Callable[[Array, float], UnivariatePotential],
                       Mt_factory: Callable[[Array, float], Dynamics],
                       Gt_factor: Callable[[Array, float], Potential],
                       N: int,
                       ...
                       Pt: Optional[Dynamics] = None):
    """
    Get a local auxiliary coupled kernel.
    All factories take as input the current value of the auxiliary variable `u_t` and the value of \sqrt{\delta/2}
    and return the resulting component of the model. Mt and Gt are supposed to take the batched version of u_{1:T}
    as input.
    ...
    def kernel(key, coupled_state, delta):
        # This uses the distributions defined below
        state_1, state_2, coupled_flags = coupled_state.state_1, coupled_state.state_2, coupled_state.coupled_flags
        x_1, ancestors_1 = state_1.x, state_1.ancestors
        x_2, ancestors_2 = state_2.x, state_2.ancestors

        sqrt_half_delta = jnp.sqrt(0.5 * delta)
        auxiliary_key, key = jax.random.split(key)
        u_1 = x_1 + sqrt_half_delta * jax.random.normal(auxiliary_key, x_1.shape)
        u_2 = x_2 + sqrt_half_delta * jax.random.normal(auxiliary_key, x_2.shape)
